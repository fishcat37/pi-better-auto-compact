...
- **Check timing matches pi native**: checked once after an agent run has fully settled (after retries, queued continuations, and native compaction checks are done) and once before a new message is sent; never mid-loop (between tool turns). Each checkpoint waits for compaction to finish before returning, and the before-send checkpoint also waits for any in-flight compaction, so a new message is only sent after compaction completes — the same ordering as native. Runs aborted by the user skip the check, and the before-send checkpoint acts as a safety net; retrying after a failed compact triggers immediately.
