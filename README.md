...
- **Check timing follows pi's lifecycle as closely as the public extension API allows**: after `turn_end`, the next `context` event checks the usage before the next LLM request, which covers the common tool-continuation case; a terminal turn has no such continuation. The extension also checks after an agent run has fully settled and before a new user message is sent. The mid-run path cannot preserve the original agent run: `ctx.compact()` is pi's manual-compaction API, so it aborts the current run. After compaction succeeds or fails, the extension sends one hidden custom message with `triggerTurn: true` to start a new continuation run. This means the old run has an `aborted` assistant record and the session contains a hidden continuation message. The event handler never waits for mid-run compaction, avoiding the `ctx.compact()`/`waitForIdle()` deadlock; the settled and before-send checkpoints still wait for compaction to finish.
...
- **Compaction uses pi's native compaction implementation, with extension-level run restart**: the same cut-point computation and default summarizer are used, with no custom instructions injected. However, `ctx.compact()` is classified by pi as manual compaction and aborts the active run; the status bar therefore shows "Compacting context...". A hidden continuation message then starts a new run after the callback, and the compaction failure path also resumes once. This is intentionally an extension-level approximation, not the same-run continuation used by pi's internal automatic compaction.
