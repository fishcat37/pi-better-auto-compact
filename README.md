...
- 本扩展不改变 pi 内置 auto-compact：内置阈值仍由 pi 在每轮 turn 结束后检查并触发。本扩展在每次 turn 结束（`turn_end`）和会话启动（`session_start`，覆盖 resume 到已超限会话的场景）读取 `ctx.getContextUsage()`，只有当**配置的扩展阈值严格低于内置阈值**（或内置已禁用）时才接管，调用 `ctx.compact()` 触发压缩。
