...
- **检查时机与 pi 原生完全一致**：agent run 完整结束后、发送新消息前各检查一次，循环中途（工具调用轮次之间）不检查、不打断；每个检查点都会等压缩完成才返回，因此发送前检查点与原生顺序相同——新消息在压缩完成后才发出。用户主动中断的 run 跳过检查，由发送新消息前的检查点兜底；compact 失败后重发消息会立即重试。
